...
public class CalculadoraController extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			// TOO aplicar logica de negocio
			calcular();

			// enviar atributos a la vista (.jsp)
			request.setAttribute("resultado", resultado);

			// request.getRequestDispatcher("views/calculadora/resultado.jsp").forward(request,
			// response);
			dispatch = request.getRequestDispatcher("views/calculadora/resultado.jsp");
		} catch (NumberFormatException e) {
			e.printStackTrace();// pinta la traza del fallo
			request.setAttribute("alert", "Por favor escribe un numero correcto");
			dispatch = request.getRequestDispatcher("views/calculadora/index.jsp");
		} catch (Exception e) {
			e.printStackTrace();// pinta la traza del fallo
			request.setAttribute("alert", "Disculpe las molestias, tenemos un fallo sin controlar");
			dispatch = request.getRequestDispatcher("views/calculadora/index.jsp");
	...
	private void getParameters(HttpServletRequest request) {
		...
		String soperador = request.getParameter("operacion");

		// TOO aplicar logica de negocio
		operador = Integer.parseInt(soperador);
		num1 = Float.parseFloat(snum1.replace(',', '.'));
		num2 = Float.parseFloat(snum2.replace(',', '.'));
		resultado = 0f;
